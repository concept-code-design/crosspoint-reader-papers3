...
void MdReaderActivity::initializeReader() {
  ...
  cachedBoldFontId       = SETTINGS.getReaderBoldFontId();
  cachedItalicFontId     = SETTINGS.getReaderItalicFontId();
  cachedBoldItalicFontId = SETTINGS.getReaderBoldItalicFontId();
  cachedMonoFontId       = SETTINGS.getReaderMonoFontId();
...
int MdReaderActivity::resolveFontId(BlockType block, uint8_t style) const {
  switch (block) {
    ...
    default:
      ...
      if ((style & BOLD) && (style & ITALIC))  return cachedBoldItalicFontId;
      if (style & BOLD)                        return cachedBoldFontId;
      if (style & ITALIC)                      return cachedItalicFontId;
...
int MdReaderActivity::runsWidth(const std::vector<InlineRun>& runs, BlockType block) const {
  ...
  for (const auto& r : runs) {
    if (!r.text.empty()) {
      total += renderer.getTextWidth(resolveFontId(block, r.style), r.text.c_str());
    }
  }
  return total;
}
  ...
  auto renderLines = [&]() {
    ...
    for (const auto& line : currentPageLines) {
      ...
      const int lineH  = renderer.getLineHeight(
          resolveFontId(line.block, line.runs.empty() ? REGULAR : line.runs[0].style));
      ...
      if (!line.runs.empty()) {
        ...
        for (const auto& run : line.runs) {
          ...
          renderer.drawText(fontId, drawX, y, run.text.c_str());
          drawX += renderer.getTextWidth(fontId, run.text.c_str());
        }
      }
