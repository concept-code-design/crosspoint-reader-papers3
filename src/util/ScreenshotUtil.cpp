...
void ScreenshotUtil::takeScreenshot(GfxRenderer& renderer) {
  ...
  if (fb) {
    ...
    if (ScreenshotUtil::saveFramebufferAsBmp(filename_str.c_str(), fb, renderer.getScreenWidth(),
  ...
  if (renderer.storeBwBuffer()) {
    renderer.drawRect(6, 6, renderer.getScreenHeight() - 12, renderer.getScreenWidth() - 12, 2, true);
